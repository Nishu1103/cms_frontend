    ...
    <div className="col-3 d-none d-md-block Dashboard_Sidedrawer px-1 ps-4 width-20">
      <div className="owned">
        ...
        {owned.length !== 0 ? (
          <>
            {owned
              .slice(0, seeAllOwned ? owned.length : Math.min(2, owned.length))
              .map((sub, index) => {
                return (
                    ...
                    <div className="Sidedrawer_Class active d-flex p-2 ps-2">
                      <div>
                        <div className="class-avatar">
                          <Avatar>
                            {sub.adminName.slice(0, 1).toUpperCase()}
                          </Avatar>
                        </div>
                      </div>
            ...
            {owned.length > 2 ? (
              <div className="See_All d-flex justify-content-end pt-2">
                {seeAllOwned ? (
                  <div onClick={() => setSeeAllOwned(false)}>See Less</div>
                ) : (
                  <div
                    onClick={() => {
                      setSeeAllOwned(true);
                      setSeeAllEnrolled(false);
                    }}
                  >
                    See All
                  </div>
          ...
          <div className="ms-2 text-muted mb-3">
            You haven't created any class.
          </div>
        )}
      </div>
        ...
        {enrolled.length !== 0 ? (
          <>
            {enrolled
              .slice(
                0,
                seeAllEnrolled ? enrolled.length : Math.min(2, enrolled.length)
              )
              .map((sub, index) => {
                return (
                  <Link key={index} to={"/classes/" + sub.classCode}>
            ...
            {enrolled.length > 2 ? (
              <div className="See_All d-flex justify-content-end pt-2">
                {seeAllEnrolled ? (
                  <div onClick={() => setSeeAllEnrolled(false)}>See Less</div>
                ) : (
                  <div
                    onClick={() => {
                      setSeeAllOwned(false);
                      setSeeAllEnrolled(true);
                    }}
                  >
                    See All
                  </div>
