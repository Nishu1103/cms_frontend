...
const ClassList = (props) => {
  ...
  const RenderClasses = () => {
    return owned.concat(enrolled).length === 0 ? (
      <div className="text-muted text-center">
        You haven't created or joined any class.
      </div>
    ) : (
      owned.concat(enrolled).map((sub, index) => {
        ...
        return (
          ...
          >
            <Link
              to={`/classes/${sub.classCode}`}
              style={{ maxWidth: "100%", minWidth: "100%" }}
            >
              <div
                className="d-none d-md-flex card class-card card-width mx-auto"
                style={backgroundStyle}
              >
                {/* <p>{sub.classCode}</p> */}
                <img src={card} className="mx-auto mt-3" height="130px" />
                <div
                  className="card-body m-3 mx-md-4 rounded-3"
                  style={{ backgroundColor: "#fff" }}
                >
                  <h5 className="card-title heading-3 text-start mb-0">
                    {sub.className}
                  </h5>
  ...
  const RenderClassesMobile = () => {
    return (activeTab === "1" ? owned : enrolled).length === 0 ? (
      <div className="text-muted text-center">
        You haven't {activeTab == "1" ? "created" : "joined"} any class.
      </div>
    ) : (
      (activeTab === "1" ? owned : enrolled).map((sub, index) => {
        ...
        return (
          <div
            key={index}
            className="col-12 col-md-6 col-lg-4 d-flex justify-content-center card-width-mobile"
          >
            <Link
              to={"/classes/" + sub.classCode}
            ...
            >
              <div
                className="d-block d-md-none card mb-4"
                style={backgroundStyle}
              >
                <div className="row g-0">
                  <div className="col-4 d-flex justify-content-center">
                    <img
                      src={card}
                      className="img-fluid rounded-start px-2"
                      width="100%"
                    />
                  </div>
                  <div className="col-8 d-flex align-items-center">
                    <div className="card-body link-wrap">
                      <h5 className="card-title heading-3 text-start px-0 mb-0">
                        {sub.className}
                      </h5>
                      <p className="card-text text-dark mb-1">
                        <span className="text-muted">{sub.adminName}</span>
                      </p>
                      <p className="card-text text-dark mobile-card-desc mb-1">
                        {sub.desc}
                      </p>
                      <a
                        href={sub.meetLink}
      ...
      <div className="row my-3 ms-0 d-block d-md-none">
        ...
        <div className="classes-navs ps-4">
          <button
            className={
              activeTab === "1"
                ? "active classes-nav-btn ps-0"
                : "classes-nav-btn ps-0"
            }
